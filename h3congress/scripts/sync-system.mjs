...
//                          vision statements/descriptions (and an H3 time; H1
//                          rows no longer carry a date — see h1AssignedYear).
//   • "H2 Database"      : one row per H2−/H2+ idea, linked back to the problem
...
// H1 statements no longer carry a date in the sheet ("H1 Time" was removed).
// To keep the Reform Map's H1 dots spread across the timeline instead of
// stacking at the axis origin, assign each a pseudo-random year in [2015, 2026].
// The value is derived from the row ID (FNV-1a) so it is stable across syncs and
// never reshuffles the map. It is intentionally NOT displayed on the Reform Map
// card — it exists only to position the dot.
function h1AssignedYear(id) {
  let h = 0x811c9dc5 >>> 0;
  for (let i = 0; i < id.length; i++) {
    h ^= id.charCodeAt(i);
    h = Math.imul(h, 16777619);
  }
  return 2015 + ((h >>> 0) % 12); // 2015..2026 inclusive
}
